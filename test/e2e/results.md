...
**Summary: 95 PASS, 4 FAIL, 0 SKIP (99 total)**

| Section | PASS | FAIL | SKIP |
|---------|------|------|------|
| 1. Discovery and Search | 11 | 4 | 0 |
...
|---|--------|--------|--------|
| 1.1 | Find lemmas about list reversal being involutive | PASS | search_by_name returned Coq.Lists.List.rev_involutive and stdpp.list_basics.reverse_involutive |
| 1.2 | Which lemmas in stdlib mention both Nat.add and Nat.mul? | PASS | search_by_symbols returned named declaration results including Nat.log2_up_wd referencing both symbols |
| 1.3 | Search for lemmas with type forall n : nat, n + 0 = n | FAIL | search_by_type returned unrelated results (DecimalFacts.app_int_nil_r, Nat.mod_0_r); did not find Nat.add_0_r |
| 1.4 | Find a lemma of type List.map f (List.map g l) = List.map (fun x => f (g x)) l | FAIL | search_by_type returned unrelated results (map_to_list, N.pow_mul_r); map_map not found |
| 1.5 | Find all commutativity lemmas in MathComp — anything matching _ * _ = _ * _ | FAIL | search_by_structure returned unrelated results (Copp_0, Qp.quarter_quarter); no multiplication commutativity lemmas |
| 1.6 | Find lemmas concluding with _ + _ <= _ | FAIL | search_by_structure returned unrelated results (ZL0, unit_enumP, polyXE); no addition/ordering lemmas |
| 1.7 | What rewrites exist for Nat.add n 0? | PASS | search_by_name returned Z.add_0_r, Z.add_0_l, N_as_OT.add_0_r, and other add-zero rewrite lemmas |
| 1.8 | What is the stdlib name for associativity of Z.add? | PASS | search_by_name returned Z.add_assoc as the canonical stdlib name |
...
| 3.2 | Which module gives me access to ssralg.GRing.Ring? | PASS | search_by_name found GRing.Ring results; list_modules confirmed mathcomp.algebra.ssralg with 10,269 declarations |
| 3.3 | What is the body of MathComp.ssrnat.leq? | PASS | get_lemma returned mathcomp.boot.ssrnat.leq with body: leq = fun m n => eq_op (m - n) 0 |
| 3.4 | If I change Nat.add_comm, what downstream lemmas break? | PASS | impact_analysis found Coq.Arith.PeanoNat.Nat.add_comm; returned root with structured output |
| 3.5 | Show me the full impact analysis for Nat.add_0_r | PASS | impact_analysis found Coq.Arith.PeanoNat.Nat.add_0_r; returned root with structured output |
| 3.6 | What Proper instances are registered for Rplus in Coquelicot? | PASS | list_instances returned 76+ Proper instances (Nat.add_wd, Nat.mul_wd, Morphisms_Prop connectives, etc.) |
...
| 4.11 | /explain-proof add_comm in examples/arith.v | PASS | explain-proof skill opened session, extracted proof trace, produced step-by-step explanation with SF passages |
...
| 5.5 | /migrate-rocq | PASS | migrate-rocq skill scanned 12 files, identified 16 deprecated From Coq imports, proposed From Stdlib replacements |
...
| 6.4 | What Coq packages are currently installed? | PASS | query_packages returned 96 installed opam packages (coq 9.1.1, coq-coquelicot 3.4.4, coq-stdpp 1.12.0, etc.) |
...
| 8.7 | Show me the Ltac call-tree breakdown for my_crush in examples/automation.v | PASS | profile_proof ltac mode returned call-tree: my_crush 100% -> reflexivity 53.8%, intros 15.4% |
...
| 8.9 | Profile all .v files in examples/ and show me the slowest files and lemmas | PASS | profile_proof timing mode on all .v files; slowest: algebra.v 0.184s, automation.v 0.101s, typeclasses.v 0.101s |

## 9. Textbook / Education RAG

| # | Prompt | Result | Reason |
|---|--------|--------|--------|
| 9.1 | /textbook how does induction work in Coq? | PASS | education_context returned 5 passages from LF > IndPrinciples, LF > IndProp |
| 9.2 | /textbook what is a proposition vs a boolean in Coq? | PASS | education_context returned 5 passages including VFA > Decide on sumbool/Prop/bool |
| 9.3 | /textbook how do I use the rewrite tactic? | PASS | education_context returned 5 passages from LF > Tactics, PLF > LibTactics |
...
| 9.5 | /textbook --volume lf what are inductively defined types? | PASS | education_context with volume=lf returned 5 passages all from LF: Basics, IndPrinciples |
| 9.6 | /textbook --volume plf what is the simply typed lambda calculus? | PASS | education_context with volume=plf returned 5 passages all from PLF: Stlc, References |
...
| 9.8 | /textbook what is the difference between assert and have? | PASS | education_context returned 5 passages from PLF > LibTactics on asserts |
| 9.9 | /textbook forall n : nat, n + 0 = n | PASS | education_context returned 5 passages from LF > Basics, LF > Induction |
| 9.10 | /textbook what is a Hoare triple? | PASS | education_context returned 5 passages from PLF > Hoare, PLF > HoareAsLogic |

---

## Remaining Issues

### Issue 1: search_by_structure returns irrelevant results for wildcard pattern queries

**Affects:** 1.5, 1.6
**Severity:** high
**Details:** `search_by_structure("_ * _ = _ * _")` returns unrelated results (Copp_0, Qp.quarter_quarter, CReal_opp_0) with no multiplication commutativity lemmas. Similarly, `search_by_structure("_ + _ <= _")` returns ZL0, unit_enumP, polyXE. The wildcard pattern matching does not discriminate on the specific operator symbols (* and +), yielding results that match only on structural shape without operator constraints.

### Issue 2: search_by_type returns poor results for well-known type signatures

**Affects:** 1.3, 1.4
**Severity:** high
**Details:** `search_by_type("forall n : nat, n + 0 = n")` returns DecimalFacts.app_int_nil_r and Nat.mod_0_r instead of Nat.add_0_r. Similarly, the `List.map` composition query returns map_to_list and N.pow_mul_r instead of map_map. The multi-channel type search is failing to rank exact or near-exact matches for common nat arithmetic and list function composition types.
