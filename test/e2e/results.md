...
Tested: 2026-03-25 (full retest of all prompts)

Run `/run-e2e` to retest prompts and update this file.

**Summary: 94 PASS, 5 FAIL, 0 SKIP (99 total)**

| Section | PASS | FAIL | SKIP |
|---------|------|------|------|
| 1. Discovery and Search | 10 | 5 | 0 |
...
|---|--------|--------|--------|
| 1.1 | Find lemmas about list reversal being involutive | PASS | search_by_name returned Coq.Lists.List.rev_involutive with score 1.24 |
| 1.2 | Which lemmas in stdlib mention both Nat.add and Nat.mul? | FAIL | search_by_symbols returned raw numeric ID/score pairs ([577, 1.0]) instead of named declaration results |
| 1.3 | Search for lemmas with type forall n : nat, n + 0 = n | FAIL | search_by_type did not return Nat.add_0_r; top results were Z.sub_0_r, Nat.mod_0_r (unrelated) |
| 1.4 | Find a lemma of type List.map f (List.map g l) = List.map (fun x => f (g x)) l | FAIL | search_by_type returned unrelated results (Z.max_min_disassoc, in_seq); map_map not found |
| 1.5 | Find all commutativity lemmas in MathComp — anything matching _ * _ = _ * _ | FAIL | search_by_structure returned raw numeric ID/score pairs ({decl_id: 96603}) instead of named declarations |
| 1.6 | Find lemmas concluding with _ + _ <= _ | FAIL | search_by_structure returned raw numeric ID/score pairs ({decl_id: 25914}) instead of named declarations |
| 1.7 | What rewrites exist for Nat.add n 0? | PASS | search_by_name returned N.add_0_l, N.add_0_r, Z.add_0_r, Nat_as_DT.add_0_r |
| 1.8 | What is the stdlib name for associativity of Z.add? | PASS | search_by_name returned Coq.ZArith.BinInt.Z.add_assoc |
| 1.9 | Does Coquelicot already have the intermediate value theorem? | PASS | search_by_name returned Coquelicot.RInt_analysis.IVT_gen_consistent and MathComp Num.poly_ivt |
| 1.10 | I need a lemma that says filtering a list twice is the same as filtering once | PASS | search_by_name returned stdpp list_filter_filter variants and Coquelicot filter_filter |
| 1.11 | Open a proof session on examples/arith.v and tell me what the %nat scope delimiter means | PASS | notation_query print_scope returned 18 nat_scope notations (+ -> Init.Nat.add, * -> Init.Nat.mul) |
...
| 1.13 | Where is Rdiv defined — Coquelicot or stdlib Reals? | PASS | search_by_name returned Coq.Reals.Rdefinitions.Rdiv as top result (stdlib Reals) |
...
| # | Prompt | Result | Reason |
|---|--------|--------|--------|
| 2.1 | /explain-error Unable to unify Nat.add ?n (S ?m) with Nat.add (S ?n) ?m | PASS | explain-error skill parsed unification error, found Nat.add_succ_r, returned structured diagnosis |
...
| 2.6 | What instances are registered for the Proper typeclass? | PASS | list_instances returned 100+ Proper instances (Nat.add_wd, Nat.mul_wd, Morphisms_Prop connectives, etc.) |
...
| # | Prompt | Result | Reason |
|---|--------|--------|--------|
| 3.1 | Show me the full definition of Coquelicot.Derive.Derive | PASS | get_lemma returned Coquelicot.Derive.Derive definition body; list_modules returned module with 203 declarations |
...
| 3.4 | If I change Nat.add_comm, what downstream lemmas break? | PASS | impact_analysis found Coq.Arith.PeanoNat.Nat.add_comm; returned root with 0 edges and sparse-result hint |
| 3.5 | Show me the full impact analysis for Nat.add_0_r | PASS | impact_analysis found Coq.Arith.PeanoNat.Nat.add_0_r; returned root with 0 edges and sparse-result hint |
| 3.6 | What Proper instances are registered for Rplus in Coquelicot? | PASS | list_instances with Coq.Classes.Morphisms.Proper returned 100+ instances |
| 3.7 | What lemmas are in the arith hint database? | PASS | inspect_hint_db returned 111 resolve entries for "arith" database (Nat.add_comm, Nat.mul_comm, Nat.le_trans, etc.) |
...
| 4.10 | /formalize For all natural numbers, addition is commutative | PASS | formalize skill produced Coq theorem (forall n m : nat, n + m = m + n) found in stdlib as Nat.add_comm |
| 4.11 | /explain-proof add_comm in examples/arith.v | PASS | explain-proof skill opened session, extracted 2-step trace, produced step-by-step explanation |
...
| 4.17 | In examples/dependent.v, which hypotheses do I need to revert before destructing n in vhead_vcons? | PASS | observe_proof_state showed hypotheses; destruct n produced 2 subgoals with type changes |
| 4.18 | Generate the convoy pattern match term for vhead in examples/dependent.v | PASS | coq_query Print vhead succeeded, returning the full convoy-pattern match term |
...
| 4.23 | Explain what Proper (eq ==> eq_set ==> eq_set) union means | PASS | tactic_lookup returned "Proper" as primitive; search_by_name returned results from Coq.Classes.Morphisms |
...
| 5.5 | /migrate-rocq | PASS | migrate-rocq skill scanned 12 files, identified 16 deprecated From Coq imports, proposed From Rocq replacements |
...
| 7.2 | Why wasn't bpow_ge_0 used by auto? | PASS | search_by_name found Flocq.Core.Raux.bpow_ge_0 |
...
| # | Prompt | Result | Reason |
|---|--------|--------|--------|
| 8.1 | Profile the proof of ring_morph in examples/algebra.v | PASS | extract_proof_trace returned 8 steps with duration_ms per step (intros, induction, reflexivity, simpl, rewrite, lia) |
...
| 8.3 | Profile examples/algebra.v and show me the top 5 slowest lemmas | PASS | profile_proof timing mode returned per-proof summaries sorted by total_time_s (ring_assoc, ring_comm, etc.) |
...
| 8.7 | Show me the Ltac call-tree breakdown for my_crush in examples/automation.v | PASS | profile_proof ltac mode returned call-tree: my_crush 100% -> reflexivity 67.4%, intros 15.6% |
...
| 8.9 | Profile all .v files in examples/ and show me the slowest files and lemmas | PASS | profile_proof timing mode called on all .v files; slowest: algebra.v 0.153s, automation.v 0.117s, lint_targets.v 0.108s |

## 9. Textbook / Education RAG

| # | Prompt | Result | Reason |
|---|--------|--------|--------|
| 9.1 | /textbook how does induction work in Coq? | PASS | education_context returned 5 passages from LF > IndPrinciples, LF > IndProp, VFA > Trie |
...
| 9.3 | /textbook how do I use the rewrite tactic? | PASS | education_context returned 5 passages from LF > Tactics (score 0.67), PLF > LibTactics |
| 9.4 | /textbook when should I use inversion vs destruct? | PASS | education_context returned 5 passages from PLF > UseTactics, LF > Tactics, LF > ProofObjects |
| 9.5 | /textbook --volume lf what are inductively defined types? | PASS | education_context with volume=lf returned 5 passages all from LF: Basics (score 0.77), IndPrinciples |
| 9.6 | /textbook --volume plf what is the simply typed lambda calculus? | PASS | education_context with volume=plf returned 5 passages all from PLF: Stlc (score 0.81), References |
| 9.7 | /textbook how do I prove things by case analysis? | PASS | education_context returned 5 passages from LF > Imp, LF > Induction, LF > Tactics |
| 9.8 | /textbook what is the difference between assert and have? | PASS | education_context returned 5 passages from PLF > LibTactics on asserts (score 0.58) |
| 9.9 | /textbook forall n : nat, n + 0 = n | PASS | education_context returned 5 passages from LF > Basics (score 0.76), LF > Induction (score 0.69) |
| 9.10 | /textbook what is a Hoare triple? | PASS | education_context returned 5 passages from PLF > Hoare (score 0.68), PLF > HoareAsLogic |

---

## Remaining Issues

### Issue 1: search_by_symbols returns raw numeric IDs instead of named declarations

**Affects:** 1.2
**Severity:** high
**Details:** `search_by_symbols(["Corelib.Init.Nat.add", "Corelib.Init.Nat.mul"])` returns `[[577, 1.0], ...]` — raw `[decl_id, score]` pairs with no declaration names, types, or modules. The tool needs to resolve numeric IDs to full declaration metadata before returning results.

### Issue 2: search_by_structure returns raw numeric IDs instead of named declarations

**Affects:** 1.5, 1.6
**Severity:** high
**Details:** `search_by_structure("_ * _ = _ * _")` returns `[{decl_id: 96603, score: 0.479}, ...]` — raw ID/score pairs with no names or types. Same root cause as Issue 1; the tool's result-resolution layer is not enriching structure search results.

### Issue 3: search_by_type returns poor results for well-known type signatures

**Affects:** 1.3, 1.4
**Severity:** high
**Details:** `search_by_type("forall n : nat, n + 0 = n")` returns Z.sub_0_r and Nat.mod_0_r instead of Nat.add_0_r. Similarly, the `List.map` composition query returns unrelated results. The multi-channel type search is failing to rank exact or near-exact matches for nat arithmetic and list function composition types.
