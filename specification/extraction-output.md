...
| Body lines | Step records | `"t": "s"` | Zero or more (one per tactic step with goals and tactic text) |
...
Each step record contains a serialized proof state, source file path, and the tactic command text that was applied. Goal-state records contain supplementary proof states not covered by any step record, for vocabulary construction.
...
> **Given** a campaign that extracts 10 proofs (30 tactic steps total, 25 with goals and tactic text) with 2 failures
> **When** the output is written
> **Then** the file contains: 1 metadata + 25 step records + supplementary goal records + 2 extraction_error + 1 summary

> **Given** a campaign with zero theorems found
> **When** the output is written
> **Then** the file contains exactly 2 lines: 1 metadata + 1 summary

#### 4.1.1 Compact Step Record
...
| `c` | string | Tactic command text applied at this step |
