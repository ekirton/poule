...
> **Deprecated (2026-04-01).** This experiment was abandoned because the Coq extraction pipeline produces only ~3,500 usable (proof_state, premises_used) training pairs from ~134,000 extracted proof records — a 97% attrition rate caused by Coq's kernel not tracking per-tactic premise usage. With ~3,500 pairs (1,600x smaller than LeanHammer's 5.8M pairs), the model cannot achieve competitive retrieval quality. The neural model was never deployed. See [neural-network-tactic-prediction.md](neural-network-tactic-prediction.md) for the replacement approach: a tactic family classifier trained on ~105,000 (proof_state, tactic) pairs from the same extraction data.
