...
The model uses a CodeBERT encoder with a closed-vocabulary tokenizer and a two-level classification hierarchy: 8 tactic categories (introduction, elimination, rewriting, hypothesis management, automation, arithmetic, contradiction, ssreflect) with within-category tactic families. Each category has a dedicated classification head. Training uses head-class undersampling to handle class imbalance: dominant tactic families are capped at a configurable maximum number of training examples, preserving all rare-family data intact. Inference produces P(tactic) = P(category) × P(tactic|category) via the product rule.
