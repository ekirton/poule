...
# ---------------------------------------------------------------------------
# Masked contrastive loss (InfoNCE with shared-positive masking)
# ---------------------------------------------------------------------------


def masked_contrastive_loss(
    state_embs: "torch.Tensor",
    premise_embs: "torch.Tensor",
    positive_indices: list[list[int]],
    temperature: float = 0.05,
) -> "torch.Tensor":
    """Compute masked contrastive loss for bi-encoder training.

    For each state, its positive premises contribute to the numerator and
    all premises that are positive for ANY state in the batch are excluded
    from that state's negative set.
    ...
    # Build positive mask: positive_mask[i, j] = 1 if premise j is positive for state i
    positive_mask = torch.zeros(N_states, N_premises, device=state_embs.device)
    ...
    # from the negative set of ALL states
    any_positive = (positive_mask.sum(dim=0) > 0).float()  # [N_premises]
    # negative_mask[i, j] = 1 if premise j is a valid negative for state i
    # A premise is a valid negative if it's not positive for ANY state
    negative_mask = 1.0 - any_positive.unsqueeze(0).expand(N_states, -1)

    # For numerical stability, subtract max
    sim_max = sim.max(dim=1, keepdim=True).values.detach()
    sim_stable = sim - sim_max

    # Denominator: exp(sim) for negatives + exp(sim) for all positives of this state
    # Include all positives in the denominator (standard InfoNCE)
    denom_mask = negative_mask + positive_mask  # valid negatives + own positives
    exp_sim = torch.exp(sim_stable) * denom_mask
    log_denom = torch.log(exp_sim.sum(dim=1, keepdim=True) + 1e-10)

    # Numerator: for each (state, positive_premise) pair, exp(sim)
    log_numerator = sim_stable  # [N_states, N_premises]

    # Per-pair loss: -log(exp(sim_pos) / sum(exp(sim_valid)))
    per_pair_loss = -(log_numerator - log_denom) * positive_mask

    # Average over all positive pairs
    total_positives = positive_mask.sum()
    if total_positives == 0:
        return torch.tensor(0.0, device=state_embs.device)

    return per_pair_loss.sum() / total_positives
