"""Impact analysis via reverse BFS."""
...
@dataclass
class ImpactSet:
    """Result of a reverse transitive closure (impact analysis)."""
    root: str
...
def impact_analysis(
    ...
    """Compute reverse transitive closure (impact set) from root via BFS over reverse_adj.

    Args:
        graph: The dependency graph.
        root: Fully qualified declaration name.
        max_depth: Maximum BFS depth (None = unlimited, <=0 clamped to 1).
        scope_filter: List of filter predicates.

    Returns:
        ImpactSet with all impacted nodes, edges, and depth map.

    Raises:
        AnalysisError: INVALID_INPUT, NOT_FOUND, or RESULT_TOO_LARGE.
    """
    if not root:
        raise AnalysisError("INVALID_INPUT", "Root declaration name must be non-empty")

    if root not in graph.forward_adj and root not in graph.reverse_adj and root not in graph.metadata:
        raise AnalysisError(
            "NOT_FOUND",
    ...
    while frontier and (max_depth is None or depth < max_depth):
        ...
        for _ in range(len(frontier)):
            ...
            for neighbor in graph.reverse_adj.get(node, set()):
    ...
    # Collect reverse edges within the impact set
    edges: set[tuple[str, str]] = set()
    for u in visited:
        for v in graph.reverse_adj.get(u, set()):
            if v in visited:
                edges.add((u, v))
